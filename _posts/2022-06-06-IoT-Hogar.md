...
<?php
                          ...
                          $estaciones_result = $conn->query("SELECT * FROM `estaciones` W  HERE `estaciones_user_id`        = ".$user_id);
			  ...
			  $alias = $_POST['alias'];
                                  ...
                                  if ($conn == true){
                                          ...
                                          $msg .= "error - " . $exito . " - " . "alias -   " . $alias . " serie - " . $serie .                              "user_id - " . $user_i  d;
                                  }
  
                                  $estaciones_result = $conn->query("SELECT * FROM `estac  iones` WHERE                                                                                               `estaciones_user_id` = ".$user_id);
                                  $estaciones = $estaciones_result->fetch_all(MYSQLI_ASSO  C);
                          }
                  ?>
...
<?php
    ...
    if ($conn==false){
      ...
      die();
          ...
          $estaciones_result = $conn->query("SELECT * FROM `estaciones` WHERE `estaciones  _user_id`=" . $user_id);
          $estaciones = $estaciones_result->fetch_all(MYSQLI_ASSOC);
  
          $consulta = "SELECT estaciones.estaciones_serie, mediciones.mediciones_date, me  diciones.mediciones_valor from mediciones inner join estaciones ON estaciones.estacione  s_id=mediciones.mediciones_estaciones_id inner join users on users.users_id=estaciones.  estaciones_user_id and users.users_id =" . $user_id . " ORDER BY DESC LIMIT 20";
...
<!DOCTYPE html>
  ...
  <head>
      ...
      <meta name="keywords"
          content="wrappixel, admin dashboard, html css dashboard, web dashboard, bootstr  ap 4 admin, bootstrap 4, css3 dashboard, bootstrap 4 dashboard, Ample lite admin bootst  rap 4 dashboard, frontend, responsive bootstrap 4 admin template, Ample admin lite dash  board bootstrap 4 dashboard template">
